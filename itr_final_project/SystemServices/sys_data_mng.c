...
/* Private variables -------------------------------------------------- */
static cbuffer_t *sdmng_conn_cbuffer[5] = { NULL };
...
{
  // Assert input
  ASSERT(conn < sizeof(sdmng_conn_cbuffer), SYS_DATA_MNG_ERROR);
  ASSERT(cb != NULL, SYS_DATA_MNG_ERROR);
...
uint32_t sys_data_mng_send(sys_data_mng_conn_t conn, uint8_t *data, uint8_t size)
{
  ...
  ASSERT(sdmng_conn_cbuffer[conn] != NULL, SYS_DATA_MNG_ERROR);

  // Check buffer size
  if (cb_space_count(sdmng_conn_cbuffer[conn]) < size)
  {
    return SYS_DATA_MNG_ERROR;
  }
